...
public class App {
    ...
    /**
     * Not particularly elegant, but we can activate different mains by
    ...
    public static void main_inMemory_datastore(String[] args) {
         ...
         * 
         * // NB: even on error, we return 200, but with a JSON object that describes
         * the error.
         * ctx.status( 200 ); // status 200 OK
         * ctx.contentType( "application/json" ); // MIME type of JSON
         * StructuredResponse resp = null;
         * 
         * // get the request json from the ctx body, turn it into SimpleRequest
         * instance
         * // NB: if gson.Json fails, expect server reply with status 500 Internal
         * Server Error
         ...
         * resp = new StructuredResponse("error", "Data with row id " + idx +
         * " not found", null);
         ...
         * message
         * // however, our "create table" sql command does specify a "NOT NULL"
         * constraint
         ...
         * "error performing insertion (title or message null?)", null);
         * } else {
     ...
     * Supports {GET, PUT, DELETE} to /messages/{id} to do associated action on
