 ...
 * @param mError  only useful when mStatus indicates an error, or when data is
 *                null
 * @param mId     any JSON-friendly object can be referenced here, so a client
 *                gets a rich reply
 */
public record StructuredResponse(String mStatus, String mError, Object mData) {
    /**
     * If the mStatus is not provided, set it to "invalid".
     * 
     * @param mStatus the status of the response, typically "ok" or "error"
     * @param mError  the message to go along with an error status
